...
# Build parser
parser = argparse.ArgumentParser(description='SOFTS-RayTune')
# Basic config
parser.add_argument('--model', type=str, required=True, default='SOFTS', help='model name, options: [SOFTS]')
...
parser.add_argument('--restore_experiment', action='store_true', default=False)
...
def build_param_space(args):
    """
    Constructs a hyperparameter search space for Ray Tune based on the specified search algorithm.
    
    This function supports the creation of search spaces for three different algorithms:
    Random Search, HyperOpt's Tree-structured Parzen Estimator (TPE), and BOHB (Bayesian Optimization with Hyperband).
    The function takes hyperparameter definitions from a JSON string and converts them into the appropriate format
    required by the specified search algorithm.

    Parameters:
    -----------
    args : argparse.Namespace
        An object containing the following attributes:
        - tune_param_space (str): A JSON string representing the hyperparameter search space. 
          This string should define a dictionary where each key is the name of a hyperparameter, 
          and each value is a list containing the distribution type and its parameters.
        - tune_search_algorithm (str): The search algorithm to be used for tuning. Supported values are:
          - "random_search": Uses Ray Tune's random search.
          - "hyperopt_tpe": Uses the TPE algorithm from the HyperOpt library.
          - "bohb": Uses the BOHB algorithm with ConfigSpace.
        - seed (int, optional): A seed value for reproducibility, only relevant when using the BOHB algorithm.
    
    Returns:
    --------
    param_space : dict or ConfigSpace.ConfigurationSpace
        The constructed hyperparameter search space:
        - If `tune_search_algorithm` is "random_search", returns a dictionary with Ray Tune distributions.
        - If `tune_search_algorithm` is "hyperopt_tpe", returns a dictionary with HyperOpt distributions.
        - If `tune_search_algorithm` is "bohb", returns a ConfigSpace.ConfigurationSpace object.
    
    Raises:
    -------
    ValueError:
        If an unsupported distribution type is encountered in the `tune_param_space`.
    
    Example:
    --------
    >>> import argparse
    >>> args = argparse.Namespace(
    >>>     tune_param_space='{"learning_rate": ["loguniform", [1e-4, 1e-2]], "batch_size": ["choice", [16, 32, 64]]}',
...
def evaluateconfig(config, args):
    """
    The Ray Tune 'trainable' function that receives a hyperparameter configuration and an argument namespace,
    executes a training loop for a deep learning model, and returns the objective cost for hyperparameter optimization.

    Parameters:
    -----------
    config : dict
        A dictionary representing the hyperparameter configuration provided by Ray Tune. This configuration may
        include values for learning rates, batch sizes, and other hyperparameters that affect the model training process.
    
    args : argparse.Namespace
        A namespace object containing command-line arguments or other configurations. This should include the 
        following attributes:
        - `seed` (int): Random seed for reproducibility across Python, PyTorch, and NumPy.
        - `tune_search_algorithm` (str): The hyperparameter search algorithm used (e.g., "hyperopt_tpe").
        - `train_epochs` (int): The number of training epochs to run.
        ...
        - `gpu` (int): Index of the GPU to use if `use_gpu` is True.
        - `use_multi_gpu` (bool): Whether to use multiple GPUs for training.
        - `device_ids` (list of int): List of GPU device IDs to use if `use_multi_gpu` is True.
        - `use_amp` (bool): Whether to use Automatic Mixed Precision (AMP) for training.
        - `model` (str): The model architecture to be used, corresponding to a key in `model_dict`.
        - `features` (str): Specifies how features are structured (e.g., "MS" for multiple series).
        - `pred_len` (int): The prediction length for the model.
        - `label_len` (int): The label length used by the model.
        - `output_attention` (bool): Whether the model outputs attention weights.
        - `data` (str): Name of the dataset being used.
        - `delta` (float): Threshold for determining improvements in validation loss (used in early stopping and checkpointing).
    
    Returns:
    --------
    None
        The function does not return a value but communicates results and checkpoints to Ray Tune using the `train.report`
        method.

    Functionality:
    --------------
    1. **Seed Initialization**: Initializes random seeds for Python, PyTorch, and NumPy to ensure reproducibility.

    2. **Configuration Unpacking**:
       - If using HyperOpt's TPE algorithm (`hyperopt_tpe`), the function unpacks the configuration dictionary using `f_unpack_dict`.
       - Certain configuration keys are cleaned up (e.g., converting `alpha_d_ff` into `d_ff` based on a multiplier).

    3. **Argument Update**:
       - Converts the `args` namespace into a dictionary, updates it with the configuration values, and converts it back to a namespace.

    4. **Device Setup**:
       - Determines whether to use GPU or CPU for training based on availability and the `args.use_gpu` flag.
       - Sets up the environment for CUDA devices if GPUs are being used.

    5. **Model Building**:
       - Constructs the model using the specified architecture from `model_dict` and moves it to the appropriate device (CPU or GPU).
       - Supports multi-GPU training if specified in `args.use_multi_gpu`.

    6. **Data Loading**:
       - Loads the training and validation datasets using `_get_data`.

    7. **Training Loop**:
       - Trains the model for the specified number of epochs (`args.train_epochs`), calculating and reporting the loss for each iteration.
       - Supports mixed-precision training (AMP) if enabled in `args.use_amp`.
       - Computes training and validation losses, updating a metrics dictionary after each epoch.

    8. **Early Stopping and Checkpointing**:
       - Implements early stopping if `args.patience` is set, monitoring the validation loss to potentially stop training early.
       - Saves checkpoints at the end of each epoch, including the model state, optimizer state, metrics, and best validation loss.
       - Restores from a checkpoint if resuming a training run, especially for multi-fidelity methods like BOHB.

    9. **Reporting to Ray Tune**:
       - After each epoch, reports training and validation metrics to Ray Tune, including the best validation loss.

    Example:
    --------
    This function is typically used within a Ray Tune optimization process where it is passed as the `trainable` function. 
    Example usage would be with a Ray Tune call like:

    >>> tune.run(evaluateconfig, config=search_space, ...)

    Note:
    -----
    - The function assumes that the necessary PyTorch model, data loading functions, and utility functions are 
      already defined elsewhere in the codebase.
    - This function is tightly integrated with the Ray Tune ecosystem and is intended to be used in conjunction 
      with hyperparameter optimization processes.
    """
    ...
    for epoch in range(start_epoch, args.train_epochs):
        ...
        valid_loss = vali(args, model, vali_loader, criterion, device)
...
def runexperiment(train_fn, args):
    """
    Run a hyperparameter tuning experiment using Ray Tune.

    This function configures and runs a hyperparameter optimization experiment using Ray Tune. 
    It allows for the specification of different search algorithms (e.g., random search, BOHB, 
    HyperOpt TPE), schedulers (e.g., FIFO, HyperBand), and resource allocations (e.g., CPU, GPU). 
    The function can resume from a previous experiment or start a new one, and it supports 
    early stopping, checkpoints, and custom stopping criteria.

    Args:
        train_fn (function): The training function to be optimized. This function should 
            accept a configuration dictionary and an argument namespace, and return the 
            objective metric (e.g., validation loss).
        args (argparse.Namespace): The argument namespace containing configuration 
            options for the experiment. This should include the following attributes:
            - `tune_cpu_resources` (int): The number of CPU resources to allocate for the tuning.
            - `tune_gpu_resources` (int): The number of GPU resources to allocate for the tuning.
            - `restore_experiment` (bool): Whether to restore an experiment from a checkpoint.
            - `tune_storage_path` (str): The path where the experiment results and checkpoints are stored.
            - `tune_experiment_name` (str): The name of the experiment to restore or create.
            - `tune_search_algorithm` (str): The algorithm to use for hyperparameter search 
              (e.g., "random_search", "bohb", "hyperopt_tpe").
            - `tune_trial_scheduler` (str): The trial scheduler to use (e.g., "fifo").
            - `tune_default_config` (str or None): JSON string of the default configuration 
              to evaluate at the start.
            - `tune_max_concurrent` (int): Maximum number of concurrently running trials.
            - `tune_objective` (str): The metric to optimize during the tuning process.
            - `train_epochs` (int): Maximum number of training epochs for each trial.
            - `seed` (int): Random seed for reproducibility.
            - `tune_num_samples` (int): Number of times to sample from the hyperparameter space.
            - `tune_time_budget_s` (int or None): Global time budget in seconds after which 
              all trials are stopped.
            - `tune_hyperopt_n_initial_points` (int): Number of initial random evaluations 
              for HyperOpt TPE.
            - `tune_hyperopt_gamma` (float): Parameter governing the Tree-structured Parzen Estimator 
              suggestion algorithm.
            - `tune_bohb_min_points_in_model` (int): Minimum number of observations to start 
              building a KDE for BOHB.
            - `tune_bohb_top_n_percent` (int): Percentage of the observations considered good 
              for BOHB.
            - `tune_bohb_num_samples` (int): Number of samples to optimize the Expected Improvement 
              for BOHB.
            - `tune_bohb_random_fraction` (float): Fraction of random configurations sampled 
              without the model for BOHB.
            - `tune_bohb_bandwidth_factor` (float): Bandwidth factor to encourage diversity 
              in BOHB.
            - `tune_bohb_min_bandwidth` (float): Minimum bandwidth to keep diversity in BOHB.

    Returns:
        ray.tune.ResultGrid: The results of the hyperparameter tuning experiment, 
            including the best configuration and associated metrics.

    Raises:
        NotImplementedError: If the specified search algorithm or scheduler is not supported.

    Notes:
        - The `train_fn` is wrapped to include the provided arguments and resources.
        - The experiment can be resumed from a previous checkpoint if `args.restore_experiment` 
          is set to True.
        - The search space is built based on the `args` configuration.
        - Depending on the search algorithm, the experiment will use the appropriate scheduler 
          (e.g., FIFO for random search or HyperOpt TPE, HyperBandForBOHB for BOHB).
        - Checkpoints are saved periodically, and the experiment can be stopped based on a 
          time budget or other custom conditions.
    """
    ...
    if args.restore_experiment:
        tuner = tune.Tuner.restore(
            path=os.path.abspath(os.path.join(args.tune_storage_path, args.tune_experiment_name)),
