# SMAC needs four core components (configuration space, 
# target function, scenario and a facade) to run an optimization process, 
...
# Build parser
parser = argparse.ArgumentParser(description='SOFTS-SMAC')
# Basic config
...
# Single argument to pass the entire Search Space as a JSON string (and also the default configuration)
...
def build_param_space(args):
    """
    Constructs a hyperparameter search space for the SMAC optimization process based on the provided JSON string of 
    parameters and their distributions.

    The function parses the JSON string from the `args.smac_param_space` argument, then uses the ConfigSpace library to 
    build a `ConfigurationSpace` object. This object is used by SMAC to perform hyperparameter optimization. The search 
    space supports different types of distributions, including categorical choices, uniform, log-uniform, and normal 
    distributions. Additionally, the function handles conditional hyperparameters, such as those dependent on a specific 
    value of another hyperparameter.

    Args:
        args (argparse.Namespace): Argument parser namespace containing the following attributes:
            - smac_param_space (str): JSON string defining the search space for hyperparameters. The string should be a 
              dictionary where keys are hyperparameter names and values are lists specifying the distribution type and 
              associated parameters.
            - seed (int, optional): Random seed for reproducibility. Default is 2021.

    Returns:
        tuple:
            - param_space (ConfigurationSpace): A ConfigSpace object defining the hyperparameter search space.
            - hps (list of str): A list of hyperparameter names included in the search space.

    Raises:
        ValueError: If an unsupported distribution type is encountered in the `args.smac_param_space` JSON string.

    Example:
        Given the following JSON string as `args.smac_param_space`:

        {
            "learning_rate": ["loguniform", [1e-5, 1e-2]],
            "batch_size": ["choice", [16, 32, 64]],
            "decomp_method": ["choice", [["moving_avg", "window_size", [2, 4, 8]], "fourier"]]
        }

        The function will construct a search space where `learning_rate` is sampled log-uniformly between 1e-5 and 1e-2, 
        `batch_size` is selected from 16, 32, or 64, and if `decomp_method` is "moving_avg", then `window_size` will be 
        sampled from 2, 4, or 8.
...
def config2dict(config: Configuration) -> dict:
    """
    Converts a SMAC Configuration object to a standard Python dictionary, ensuring compatibility by converting 
    numpy integer types to base Python integers.

    This function is useful for preparing the configuration output from SMAC for further processing or storage 
    where standard Python data types are required.

    Args:
        config (Configuration): A Configuration object from SMAC representing a set of hyperparameter values.

    Returns:
        dict: A dictionary where the keys are hyperparameter names and the values are their corresponding 
        values from the Configuration object. Any numpy integer types are converted to base Python integers 
        to avoid runtime errors.

    Example:
        Given a SMAC `Configuration` object with hyperparameters:

        Configuration:
        learning_rate = 0.001
        batch_size = 32 (numpy.int64)

        The function will return:

        {
            "learning_rate": 0.001,
            "batch_size": 32  # as a standard Python int
        }
    """
...
def train(config: Configuration, seed: int = 0, budget: int = 5) -> float:
    """
    Trains a model using a given hyperparameter configuration and reports the best validation loss. This function 
    is designed to be used as the target function in a hyperparameter optimization process with SMAC.

    Args:
        config (Configuration): A SMAC Configuration object containing the hyperparameters to be used for training.
        seed (int, optional): A random seed for reproducibility. Defaults to 0.
        budget (int, optional): The number of epochs to train for (used as the budget for multi-fidelity optimization). 
                                Defaults to 5. 
    ...
    Example:
        The `train` function is intended to be used in a hyperparameter tuning loop with SMAC. It will load 
        the dataset, initialize the model with the given hyperparameters, and run training for a specified number 
        of epochs (budget). The function reports the best validation loss, which SMAC uses to evaluate the configuration.
    """
    # parse args
    ...
    print("train_epoch", round(budget))
    # Training loop
    for epoch in range(start_epoch, round(budget)):
        iter_count = 0
        train_loss = []
        model.train()
        ...
        # Update state of Learning Rate Scheduler
...
if __name__ == "__main__":
    ...
    facade = MFFacade(
        ...
        overwrite=(not args.restore_experiment), 
        )
    ...
    for trial_info, trial_value in facade.runhistory.items():
        ...
        budget = round(trial_info.budget)
